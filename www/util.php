...
function store_credentials($user_id, $credentials) {
  ...
  if(get_credentials($user_id) != NULL) {
    $is_new = FALSE;
  }
...
function bootstrap_new_user($is_new = TRUE) {
  global $base_url;
  ...
  if($is_new) { 
    ...
    $message_to_send = "<article class='auto-paginate'>"
                          ."<p class='text-small' style='border-bottom: 1px solid #666'>"
                            ."<span>Irssi</span>"
                            ."<span class='yellow'>Glass</span>"
                            ."<span class='blue'>&lt;SYSTEM&gt;</span>"
                          ."</p>"
                          ."<p class='section text-normal'>Thanks for setting up IrssiGlass!</p>"
                          ."<p class='section text-normal'>Questions/Comments? Send to brbuller@gmail.com</p>"
                        ."</article>";
    $new_timeline_item = new Google_TimelineItem();
